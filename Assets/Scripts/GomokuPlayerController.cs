...
public class GomokuPlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(pv.IsMine)
        {
            GameObject.FindObjectOfType<GomokuNetworkManager>().SetOurSideText(pieceColor);
        }
        else
        {
            GameObject.FindObjectOfType<GomokuNetworkManager>().SetOtherSideText(pieceColor);
        }
    }
    ...
    void Update()
    {
        ...
        // If player is not click ready
        var players = FindObjectsOfType<GomokuPlayerController>();
        foreach(var player in players)
        {
            if (playerState != PlayerState.Start) return;
        }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                // Game Over
                GameObject.FindObjectOfType<GomokuNetworkManager>().gameObject.GetComponent<PhotonView>().RPC("GameOver", RpcTarget.All, pieceColor);
            }
    ...
    [PunRPC]
    public void SetReadyAndStart()
    {
        playerState = PlayerState.Start;
        if(pv.IsMine)
        {
            GameObject.FindObjectOfType<GomokuNetworkManager>().ourSideReadyText.text = "Start";
        }
        else
        {
            GameObject.FindObjectOfType<GomokuNetworkManager>().otherSideReadyText.text = "Start";
        }
    }
}
